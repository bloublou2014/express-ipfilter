...
let ipfilter = require('../index').IpFilter;
let IpDeniedError = require('../index').IpDeniedError;
...
describe('using ranges', function () {
  describe('enforcing whitelist restrictions', function () {
    beforeEach(function () {
      // Ip range: 127.0.0.1 - 127.0.0.14
      this.ipfilter = ipfilter([['127.0.0.1', '127.0.0.14']], {
        log: false,
        mode: 'allow',
  ...
  describe('check range restrictions with only one ip in the range', function () {
    it('should throw an error if ip range is static', function () {
      let init = function () {
        ipfilter([['127.0.0.1']], {log: false, mode: 'allow'});
      };
      assert.throws(init, Error);
    });

    it('should throw an error if ip range is dynamic', function () {
      let req = {
        session: {},
        headers: [],
        connection: {
      ...
      let ipfilterInstance = ipfilter(function dynamicRange() {
        return [['127.0.0.1']];
      }, {log: false, mode: 'allow'});

      let filter = function () {
        ipfilterInstance(req, {}, function () {
          assert.fail(true, false, 'Should not passed');
        });
      };
      assert.throws(filter, Error);
    });
  });
...
describe('external logger function', function () {
  ...
  it('should log to a passed logger exactly one message', function (done) {
    let messages = [];
    let logF = function logFF(message) {
...
describe('LogLevel function', function () {
  ...
  it('should log allow if log level is set to allow and mode allow', function (done) {
    let messages = [];
    let logF = function logFF(message) {
      messages.push(message);
    };
    this.ipfilter = ipfilter(['127.0.0.2'], {log: true, logF: logF, logLevel: 'allow', mode: 'allow'});
    this.req = {
      session: {},
      headers: [],
      connection: {
  ...
  it('should not log allow if log level is set to deny and mode allow', function (done) {
    let messages = [];
    let logF = function logFF(message) {
      messages.push(message);
    };
    this.ipfilter = ipfilter(['127.0.0.2'], {log: true, logF: logF, logLevel: 'deny', mode: 'allow'});
    this.req = {
      session: {},
      headers: [],
      connection: {
...
describe('mixing different types of filters', function () {
  describe('with a whitelist', function () {
    beforeEach(function () {
      this.ipfilter = ipfilter(['127.0.0.1', '192.168.1.3/28', ['127.0.0.3', '127.0.0.35']], {
        cidr: true,
        mode: 'allow',
        log: false
      });
...
describe('mixing different types of filters with IPv4 and IPv6', function () {
  ...
  let ips = ['127.0.0.1', '192.168.1.3/28', '2001:4860:8006::62', '2001:4860:8007::62/64', ['127.0.0.3', '127.0.0.35']];
...
describe('using ips as a function', function () {
  ...
  describe('with a blacklist', function () {
    ...
    it('should deny', function (done) {
      this.req.connection.remoteAddress = '127.0.1.1';
      checkError(this.ipfilter, this.req, done);
    });
  });
});
